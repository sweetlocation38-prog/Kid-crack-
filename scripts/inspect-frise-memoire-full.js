...
const NIVEAUX_FAIBLES = {
  frise_temps: ['ce1-2','ce1-3','ce2-1','ce2-2','ce2-3','cp-1','cp-2','cp-3','gs-2','gs-3','ms-2','ms-3'],
  memoire_etoiles: ['ce1-1','ce1-2','ce1-3','ce2-1','ce2-2','ce2-3','cp-1','cp-2','cp-3','gs-1','gs-2','gs-3','ms-3'],
};
...
async function main() {
  ...
  for (const code of Object.keys(NIVEAUX_FAIBLES)) {
    ...
    for (const row of data) {
      ...
      if (!NIVEAUX_FAIBLES[code].includes(cle)) continue;
      if (!parNiveau[cle]) parNiveau[cle] = [];
      parNiveau[cle].push(row.donnees);
    }
    resultat[code] = parNiveau;
  }
  fs.writeFileSync('scripts/frise-memoire-full.json', JSON.stringify(resultat, null, 2));
  console.log('OK');
}
